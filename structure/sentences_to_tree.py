...
# TODO: rename model filenames to matcht their name in markov.py
# rename methods to match their name in markov.py
# a method to turn a parse tree into a deep list (so I can print input sentences and their parses for grepping)
# a model with parent word, left word and tag.
...
nlp = spacy.load('en_core_web_md') # TODO use en_core_web_lg 
...
def to_parse_tree(root):
  return [to_parse_tree(child) for child in root.lefts] + [root] + [to_parse_tree(child) for child in root.rights]
